...
def send_data(data):
    ...
    csm2 = checksum2(csm1)
    data.insert(0, 0xFF) #add headers
  
    data.insert(1, 0xFF)
    
    data.insert(5, csm1) #add checksum1 & 2 at appropriate positions in the 
...
def get_model(servoid):
    ...
    """
    data = []
    data.append(0x09) #packet size(=9)
    data.append(servoid)
    data.append(EEP_READ_REQ) #EEP_READ(CMD)
    data.append(MODEL_NO1_EEP) #(=0) data[0], i.e. start from EEP ADDR 0 
    data.append(BYTE1) #(=0X01=1) data[1] signifies no. of bytes to be read from data[0] 
    send_data(data)
    rxdata = []
    try:
